...
// دیتای جامع احساسات (بر اساس مدل How We Feel / Mood Meter)
const emotionGrid = {
  yellow: {
...
export default function MindfulApp() {
  ...
  const loadAllData = async () => {
    try {
      ...
      // Load value ratings
      const { data: valueRatingsData } = await supabase
        .from('value_ratings')
        .select('*')
        .order('created_at', { ascending: false })
        .limit(30);
  ...
  // دیتای تستی برای نمودار پیشرفت (داینامیک بر اساس احساس امروز)
  const progressData = [
        ...
        <div className="flex justify-between items-center max-w-3xl mx-auto relative z-10">
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-3 text-center border border-white/10">
            ...
            <p className="text-white text-xl font-black mt-1 flex items-center justify-center gap-1">
              🔥 {reportStats.meditation.totalDays} <span className="text-sm font-normal">روز</span>
            </p>
          </div>
        </div>
      </div>
